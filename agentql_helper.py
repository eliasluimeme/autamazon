...
def query_and_extract(page, query: str, cache_key: str = None) -> dict:
    ...
    Args:
        page: Playwright page object (will be wrapped if needed)
        query: AgentQL query string
        cache_key: Optional key to cache the extracted selectors (persists to disk)
        
    Returns:
        Dict with element names as keys and {element, xpath, css, locator} as values
    """
    ...
    try:
        logger.info(f"🧠 Querying AgentQL for: {cache_key or 'dynamic query'}...")
        ...
        for attr_name in dir(response):
            ...
            # Extract XPath and CSS if dompath is available
            if DOMPATH_AVAILABLE:
                ...
                try:
                    results[attr_name]['css'] = css_path(element)
                except:
                    pass
